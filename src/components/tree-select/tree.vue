  ...
  <div class="ats-tree" v-clickoutside="handleCloseTree" ref="atsTree">
    <div class="ats-input" @mouseenter="hovering=true" @mouseleave="hovering=false">
      <div class="ats-input-single" v-if="!this.multiple">
        ...
        <input type="text" class="el-input__inner" v-model="treeSelected" :placeholder="placetext" @input="handleFilter" @focus="treeVisible=true"
      ...
      <div class="ats-input-multiple el-input__inner" v-if="this.multiple">
        ...
        <div class="ats-labels">
          <div class="el-select__tags" @click.prevent="handleCloseTree(true)">
            <el-tag v-for="item in checkedItems" :key="item.id" :closable="true" type="primary" class="el-tag--primary" @close="handleDelItem(item,$event)"
              :title="handleTitleVisible(item[treeProps.label])">
              {{item[treeProps.label] | showEllips}}
            </el-tag>
          </div>
          <input ref="multipleInput" type="text" :placeholder="placetext" v-model="treeSelected" @input="handleFilter" @focus="treeVisible=true"
            @blur="handleAutoComplete">
        </div>
      </div>
    </div>
    <el-scrollbar v-show="treeVisible" class="ats-tree-scrollbar" ref="treeScrollbar">
      <div class="ats-tree-wrapper">
        <ul class="ats-tree-nodes">
          <tree-node v-for="child in treeNodes[treeProps.children]" :node="child" :key="child.id" :multiple="multiple" :currentNodeId="currentNodeId"
            :treeProps="treeProps" :eventHub="eventHub" :query="query" :isQuering="isQuering">
          </tree-node>
        </ul>
      </div>
    </el-scrollbar>
...
export default {
  ...
  methods: {
    ...
    treeFilterMethod(node) {
      ...
      if (!node.visible && childNodes.length > 0) {
        let allHidden = true;
        childNodes.forEach(child => {
