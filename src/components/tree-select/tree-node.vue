<template>
  <li class="ats-tree-node" :class="{'is-expanded':node.expanded}" @click.stop="handleClick(node,$event)">
    <div class="ats-tree-node__content" v-show="node.visible" :style="{ 'padding-left': (node[treeProps.level] - 1) * 16 + 'px' }"
    ...
    <ul class="ats-tree-node__children" v-if="node.expanded">
      <tree-node v-for="child in node[treeProps.children]" :node="child" :key="child.id" :treeProps="treeProps" :currentNodeId="currentNodeId"
        :eventHub="eventHub" :query="query" :isQuering="isQuering">
      </tree-node>
    </ul>
  </li>
</template>
